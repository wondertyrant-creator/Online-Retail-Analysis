...
-- ============================================================
-- EXPLORATORY CHECK: extreme values in raw data
-- Run before cleaning to catch anomalies in their original form.
-- These two queries flagged stockcodes 23843 and 23166 as having
-- unusually large quantities (80,995 and 74,215 units). Investigation
-- confirmed both were orders cancelled within minutes of being placed
-- (see clean_orders section for how cancellations net these to zero).
-- ============================================================

SELECT stockcode, description, quantity, unitprice,
       CAST(quantity AS SIGNED) * CAST(unitprice AS DECIMAL(10,2)) AS line_value
FROM raw_orders
...
SELECT stockcode, description, quantity, unitprice,
       CAST(quantity AS SIGNED) * CAST(unitprice AS DECIMAL(10,2)) AS line_value
FROM raw_orders
ORDER BY CAST(quantity AS SIGNED) * CAST(unitprice AS DECIMAL(10,2)) DESC
LIMIT 15;
-- ============================================================
...
SELECT
    ...
    ROUND(CAST(quantity AS DECIMAL(10,2)) * CAST(unitprice AS DECIMAL(10,2)), 2) AS revenue,
    CASE WHEN customerid IS NULL OR TRIM(customerid) = '' THEN 'Guest' ELSE TRIM(customerid) END AS customer_id,
    TRIM(country) AS country,
    CASE WHEN ABS(CAST(quantity AS SIGNED)) > 5000 THEN 1 ELSE 0 END AS is_outlier
FROM raw_orders
...
CREATE INDEX idx_clean_is_outlier  ON clean_orders(is_outlier);
...
SELECT
    ...
    ROUND(
        SUM(revenue)
        / NULLIF(COUNT(DISTINCT CASE WHEN invoiceno NOT LIKE 'C%' THEN invoiceno END), 0)
    , 2)                                                                         AS avg_order_value,
    ROUND(
        SUM(CASE WHEN invoiceno NOT LIKE 'C%' THEN quantity ELSE 0 END)
        / NULLIF(COUNT(DISTINCT CASE WHEN invoiceno NOT LIKE 'C%' THEN invoiceno END), 0)
...
SELECT
    ...
    ROUND(units_returned * 100.0 / NULLIF(units_sold, 0), 1)       AS return_rate_pct
FROM product_totals
WHERE units_sold > 0
...
WITH monthly AS (
    SELECT
        `year_month`,
        COUNT(DISTINCT CASE WHEN invoiceno NOT LIKE 'C%' THEN invoiceno END) AS total_orders,
        COUNT(CASE WHEN invoiceno NOT LIKE 'C%' THEN 1 END)                  AS total_lines,
        SUM(CASE WHEN invoiceno NOT LIKE 'C%' THEN quantity ELSE 0 END)      AS total_items,
...
SELECT
    `year_month`,
    total_orders,
    ROUND(total_items / NULLIF(total_orders, 0), 1)   AS avg_items_per_order,
    ROUND(total_lines / NULLIF(total_orders, 0), 1)   AS avg_lines_per_order,
...
-- ============================================================
-- ONLINE RETAIL ANALYSIS — STAR SCHEMA LAYER
-- Run AFTER the original script has built: clean_orders, dim_date
--
-- Replaces these chart-specific views (safe to DROP once Power BI
-- is repointed at the star schema):
--   vw_monthly_revenue, vw_top_products, vw_top_customers,
--   vw_revenue_by_country, vw_customer_segments, 
--   vw_avg_order_value, vw_product_return_rate,
--   vw_basket_size_by_month, vw_basket_size_by_country,
--   vw_revenue_concentration, vw_guest_vs_identified
--
-- Keeps: clean_orders (source), dim_date,
--        vw_data_quality_summary (needed for the "Rows Retained %" measure),
--        vw_rfm (vw_dim_customer builds on top of this — do not drop)
-- ============================================================
...
-- description picked as the MOST FREQUENT description seen for
-- that stock_code (source data has minor spelling/case drift).
-- is_product = 0 flags postage/fees/manual adjustment codes so
-- "Top Products" / "Units Returned" visuals can filter them out
-- without losing them from revenue totals.
-- ============================================================
...
SELECT
    dc.stockcode                                                AS stock_code,
    dc.description,
    CASE
        WHEN dc.stockcode IN (
            'POST','DOT','M','m','C2','B','S',
            'AMAZONFEE','BANK CHARGES','PADS'
        ) OR dc.stockcode LIKE 'gift_%' THEN 0
        ELSE 1
    END                                                          AS is_product
...
-- ============================================================
-- DIM: GEOGRAPHY
-- Grain: one row per country. Kept as its own table (rather than
-- a degenerate attribute on fact_orders) purely for star-schema
-- ============================================================
 
DROP VIEW IF EXISTS vw_dim_geography;
 
CREATE VIEW vw_dim_geography AS
SELECT DISTINCT
    country
FROM clean_orders;
...
-- ============================================================
-- DIM: CUSTOMER
-- Grain: one row per customer_id (including the single 'Guest'
-- bucket). Carries everything that needs to be STATIC regardless
-- of report date-range filters:
--   - customer_type   : One-time / Repeat buyer, based on ALL-TIME
--                        order count (not just the selected range)
--   - revenue_decile  : 1 = top 10% of spend, 10 = bottom 10%
--   - rfm_segment etc.: Champions / Loyal / At Risk / Inactive / Recent
-- Guest gets is_guest = 1 and NULL RFM/decile/customer_type, since
-- it represents many different anonymous people, not one person.
-- ============================================================
...
WITH customer_base AS (
    SELECT DISTINCT customer_id FROM clean_orders
),
customer_metrics AS (
    SELECT
        customer_id,
...
joined AS (
    SELECT
        cb.customer_id,
        CASE WHEN cb.customer_id = 'Guest' THEN 1 ELSE 0 END        AS is_guest,
        cm.order_count                                              AS total_orders,
        cm.lifetime_revenue,
        CASE
            WHEN cb.customer_id = 'Guest'   THEN NULL
            WHEN cm.order_count <= 1        THEN 'One-time buyer'
            ELSE                                  'Repeat buyer'
    ...
    LEFT JOIN vw_rfm            r ON cb.customer_id = r.customer_id
)
SELECT
    *,
    CASE
        WHEN monetary IS NOT NULL
        THEN NTILE(10) OVER (ORDER BY CASE WHEN monetary IS NOT NULL THEN monetary END DESC)
        ELSE NULL
    END AS revenue_decile
FROM joined;
 
-- ============================================================
-- FACT: ORDERS
-- Grain: one row per invoice line — same grain as clean_orders.
-- Cancellation lines are KEPT (not filtered out) and flagged via
-- is_cancellation, because their negative quantity is what makes
-- revenue net out correctly when summed in DAX.
-- ============================================================
...
SELECT
    ...
    unit_price,
    revenue,
    CASE WHEN invoiceno LIKE 'C%' THEN 1 ELSE 0 END             AS is_cancellation
FROM clean_orders;
 
 
-- ============================================================
-- NOTES ON IMPORTING INTO POWER BI
-- ============================================================
-- Import: vw_fact_orders, vw_dim_customer, vw_dim_product,
--         vw_dim_geography, dim_date, vw_data_quality_summary
--
-- Relationships (all single-direction, 1-to-many from dim to fact):
...
--
-- Performance note: vw_dim_customer recomputes RFM/decile window
-- functions across the full order history on every Power BI
-- refresh. For a dataset this size (~25K rows) it's negligible,
-- but if you ever materialize this for a larger dataset, swap
-- "CREATE VIEW" for "CREATE TABLE AS" on vw_dim_customer and
-- vw_fact_orders, then add:
--   CREATE INDEX idx_fact_customer ON fact_orders(customer_id);
--   CREATE INDEX idx_fact_product  ON fact_orders(stock_code);
--   CREATE INDEX idx_fact_country  ON fact_orders(country);
--   CREATE INDEX idx_fact_date     ON fact_orders(order_date);
-- ============================================================
 SELECT * FROM vw_dim_product;
 SELECT * FROM vw_dim_geography;
 SELECT * FROM vw_dim_customer;
 SELECT * FROM vw_fact_orders;
-- ============================================================
-- SHOWCASE QUERIES
-- ============================================================
 
-- Sanity check: every fact row should resolve to a dim_customer row
SELECT COUNT(*) FROM vw_fact_orders f
LEFT JOIN vw_dim_customer c ON f.customer_id = c.customer_id
WHERE c.customer_id IS NULL;
-- Should return 0
 
-- Sanity check: every fact row should resolve to a dim_product row
SELECT COUNT(*) FROM vw_fact_orders f
LEFT JOIN vw_dim_product p ON f.stock_code = p.stock_code
